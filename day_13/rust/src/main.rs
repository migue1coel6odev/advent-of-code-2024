...
fn main() {
    ...
    let token_sum = vec_machines
        .iter()
        .map(|f| f.calculate_best_token_usage())
        .sum::<usize>();
...
fn read_file() -> Vec<Machine> {
    ...
    for capture in captures {
        ...
        let button_a = (
            capture["a_x"].parse::<usize>().unwrap(),
            capture["a_y"].parse::<usize>().unwrap(),
        );
        let button_b = (
            capture["b_x"].parse::<usize>().unwrap(),
            capture["b_y"].parse::<usize>().unwrap(),
        );
        let prize = (
            capture["prize_x"].parse::<usize>().unwrap(),
            capture["prize_y"].parse::<usize>().unwrap(),
        );

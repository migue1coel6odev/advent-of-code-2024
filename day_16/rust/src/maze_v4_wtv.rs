...
pub struct Mazev4<'a, 'b> {
    ...
    queue: HashMap<(Vec<Vec<char>>, (usize, usize), Facing), (usize, bool)>,
}
...
impl<'a, 'b> Mazev4<'a, 'b> {
    ...
    pub fn find_best_path(&mut self) -> Vec<usize> {
        self.queue.insert(
            (self.map.clone(), self.start_pos, self.start_facing),
            (0, false),
        );

        loop {
            let new_queue = self.sort_queue_by_cost();
            if new_queue.len() == 0 {
                break;
            }

            if let Some(item) = new_queue.get(0) {
                self.next_path(item.0 .0.clone(), item.0 .1, item.0 .2, item.1);
                self.queue
                    .entry(item.0.clone())
                    .and_modify(|(_, checked)| *checked = true);
            }
        }
        ...
        self.sort_by_closer_to_end(elems)
            ...
            .for_each(|(start_facing, start_pos, character)| {
                if character != &'X' {
                    ...
                    if start_pos.0.cmp(&current_pos.0) != Ordering::Less
                        || start_pos.1.cmp(&current_pos.1) != Ordering::Greater
                    {
                        let key = (new_map, *start_pos, *start_facing);
                        let mut value =
                            cost + Mazev4::check_facing_cost(&facing, &start_facing) + 1;
                        self.queue
                            .entry(key)
                            .and_modify(|(f, checked)| {
                                if f > &mut value {
                                    *f = value;
                                    *checked = false;
                                }
    ...
    fn sort_queue_by_closer_to_end(
        &mut self,
    ) -> Vec<((Vec<Vec<char>>, (usize, usize), Facing), usize)> {
        let mut to_array = self
            .queue
            .iter()
            .filter_map(|(f, x)| if !x.1 { Some((f.clone(), x.0)) } else { None })
            .collect::<Vec<((Vec<Vec<char>>, (usize, usize), Facing), usize)>>();
        to_array.sort_by(|((_, pos_a, _), _), ((_, pos_b, _), _)| {
            if check_euclidian_distance_between_points(&self.end_pos, &pos_a)
                > check_euclidian_distance_between_points(&self.end_pos, &pos_b)
            {
                Ordering::Greater
    ...
    fn sort_queue_by_cost(&mut self) -> Vec<((Vec<Vec<char>>, (usize, usize), Facing), usize)> {
        let mut to_array = self
            .queue
            .iter()
            .filter_map(|(f, x)| if !x.1 { Some((f.clone(), x.0)) } else { None })
            .collect::<Vec<((Vec<Vec<char>>, (usize, usize), Facing), usize)>>();
        to_array.sort_by(
            |((_, _, _), cost_a), ((_, _, _), cost_b)| match cost_b.cmp(cost_a) {

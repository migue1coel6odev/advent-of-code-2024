...
impl Maze {
    ...
    fn next_path(
        ...
        let surroundings = check_surroundings_char(
            ...
            start_pos,
            vec!['.', 'E'],
            vec![Maze::map_matrix_pos_inverted(&start_facing)],
        );
        ...
        if surroundings.len() == 1 {
            Maze::mark_as_visited(&mut self.map, &start_pos);
        }
        ...
        surroundings
            ...
            .for_each(|(matrix_pos, pos, character)| {
                if character == &'E' {
                    ...
                    let new_facing = Maze::map_facing(matrix_pos);
                    let facing_cost = Maze::check_facing_cost(&start_facing, &new_facing);
                    self.queue
